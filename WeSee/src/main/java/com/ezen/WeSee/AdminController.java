...
public class AdminController {
	...
	public String doSearch(HttpServletRequest req) {
		...
		String ret = adminMapper.searchMember(dto);
		...
		if(ret == null) {
			...
			if(email != null && !(email.equals(""))) {
		        ...
		        int passRes = adminMapper.ChangePasswd(String.valueOf(checkNum), email);
		        ...
		        if(passRes > 0) {
			        ...
			        String content = 
			        		req.getParameter("name") + "님! 요청에 따라 비밀번호가 변경되었습니다." +
			                "<br><br>" + 
		        	...
		        	msg = "이메일 발송에 실패했습니다. 관리자에게 문의해 주세요.";
					url = "goLogin.do";
		        }
			}else {
				msg = "회원님의 이메일은 "+ret+"입니다.";
				url = "goLogin.do";
			}
		}
		req.setAttribute("msg", msg);
	...
	public String goAdminGoodsUpdate(HttpServletRequest req, @RequestParam int gnum) {
		...
		String gcategory[] = new String[] {"ecobag", "cup", "poster", "griptok", "keyring", "filmmark", "stationery", "etc"};
	...
	public String doAdminGoodsUpdate(HttpServletRequest req, @ModelAttribute GoodsDTO dto, 
		...
		if(gimage == null || gimage.trim().equals("")) {
			gimage = mr.getParameter("gimage2");
		}
